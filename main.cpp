	...
	while (true)
	{
		...
		do
		{
			...
			if (TimeNow() - timeInit > SLEEP)
			{
				...
				if (Cacti.x == OBSTX && Cacdi.x == OBSTX && Cactri.x == OBSTX)
				{
					frontCactus = 0;
				}
				else if (Cacti.x < Cacdi.x && Cacti.x < Cactri.x)
				{
					frontCactus = 0;
				}
				else if (Cacdi.x < Cactri.x && Cacdi.x < Cacti.x)
				{
					frontCactus = 1;
				}
				else if (Cactri.x < Cacti.x && Cactri.x < Cacdi.x)
				{
					frontCactus = 2;
				}
				...
				if (Cacti.x == OBSTX)
				{
					Cacti.frame = randomframe();
				}
				if (Cacdi.x == OBSTX)
				{
					Cacdi.frame = randomframe();
				}
				if (Cactri.x == OBSTX)
				{
					Cactri.frame = randomframe();
				}
				...
				else if (frontCactus == 1)
				{
					obstacledraw(Cacdi.theme, Cacdi.frame, Cacdi.x, Cacdi.y);
				}
				else if (frontCactus == 2)
				{
					obstacledraw(Cactri.theme, Cactri.frame, Cactri.x, Cactri.y);
				}
